...
function updateKnowledgeVesselMeta(lang) {
    const title = translations[lang]?.['knowledgeVessel.seo.title'];
    const desc = translations[lang]?.['knowledgeVessel.seo.desc'];
    if (title) {
...
document.addEventListener('DOMContentLoaded', () => {
    initLanguageSwitcher({
        afterSet: (lang) => {
            updateKnowledgeVesselMeta(lang);
