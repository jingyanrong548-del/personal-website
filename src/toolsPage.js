...
function updateToolsMeta(lang) {
    const title = translations[lang]?.['tools.seo.title'] || 'Engineering Suite — Open Thermal AI';
    const desc = translations[lang]?.['tools.seo.desc'] || '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initLanguageSwitcher({
        afterSet: (lang) => {
            updateToolsMeta(lang);
            refreshWhatsNewLanguage(lang);
            refreshAppsFilterFromUI();
        },
    });
    updateToolsMeta(getCurrentLanguage());
