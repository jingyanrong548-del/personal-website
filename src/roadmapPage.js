...
function updateRoadmapMeta(lang) {
    const title = translations[lang]?.['roadmap.seo.title'] || 'Open Thermal AI Roadmap';
    const desc = translations[lang]?.['roadmap.seo.desc'] || '';
...
document.addEventListener('DOMContentLoaded', () => {
    initLanguageSwitcher({
        afterSet: (lang) => {
            updateRoadmapMeta(lang);
            refreshWhatsNewLanguage(lang);
        },
    });
    updateRoadmapMeta(getCurrentLanguage());
